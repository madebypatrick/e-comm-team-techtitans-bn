...
const isAdmin = async (req, res, next) => {
  ...
  try {
    ...
    if (user && decodedToken && decodedToken.role === 1) {
      ...
      res.status(403).json({ message: 'Your are Unauthorized to perform this action' });
    }
  } catch (err) {
    console.error(err);
...
const isSeller = async (req, res, next) => {
  ...
  try {
    const decodedToken = JwtUtility.verifyToken(token);
    const user = await User.findOne({ where: { uuid: decodedToken.uuid } });
    if (user && decodedToken && decodedToken.role === 2) {
      next();
    } else {
      res.status(403).json({ message: 'Your are Unauthorized to perform this action' });
    }
    ...
    console.error(err);
    res.status(500).json({ message: 'Your are Unauthorized to perform this action' });
  }
};
